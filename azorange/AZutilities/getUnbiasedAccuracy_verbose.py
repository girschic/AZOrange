...
from AZutilities import evalUtilities_verbose
        ...
        if not statistics or len(statistics) < 1:
            ...
            if len(consensusMLs) >= 2:
                ...
                for foldN in range(self.nExtFolds):
                    ...
                    #Test the model
                    if self.responseType == "Classification":
                        Cresults.append((evalUtilities_verbose.getClassificationAccuracy(testData, model), evalUtilities.getConfMat(testData, model) ) )
                    else:
                        local_exp_pred = []
                        for ex in testData:
                            local_exp_pred.append((ex.getclass(), model(ex)))
